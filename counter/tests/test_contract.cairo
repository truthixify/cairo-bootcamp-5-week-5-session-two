use counter::Counter::{
    ContractFunded, CounterDecremented, CounterIncremented, CounterReset, Event, RewardClaimed,
};
...
const STRK_TOKEN: ContractAddress =
    ...
    .try_into()
    .unwrap();
const ADMIN: ContractAddress = 0x021a084dd1ebdf8fa827f2c89f15e95b52975101bbbdd8d2d8711fa8aae84c07
    .try_into()
    .unwrap();
    ...
    spy
        .assert_emitted(
            @array![
                ...
                (
                    contract.contract_address,
                    Event::ContractFunded(ContractFunded { amount: reward_amount }),
                ),
            ],
        );
}
...
fn test_decrement_to_zero_claims_reward() {
    ...
    assert_eq!(
        user_balance_after, user_balance_before + reward_amount, "User should receive reward",
    );
    ...
    spy
        .assert_emitted(
            @array![
                (
                    contract.contract_address,
                    Event::RewardClaimed(RewardClaimed { winner: USER, amount: reward_amount }),
                ),
            ],
        );
}
